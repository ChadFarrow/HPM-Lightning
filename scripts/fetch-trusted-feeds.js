...
// Search for The Trusted feeds
async function fetchTheTrustedFeeds() {
  try {
    const headers = generateAuthHeaders();
    
    // Search for music feeds by The Trusted
    const searchUrl = `${API_URL}/search/music/byterm?q=the+trusted&max=20`;
    
    console.log('Fetching The Trusted feeds from Podcast Index...');
    ...
    if (!data.feeds || data.feeds.length === 0) {
      ...
      const regularSearchUrl = `${API_URL}/search/byterm?q=the+trusted&val=music&max=20`;
...
// Main function
async function main() {
  const feeds = await fetchTheTrustedFeeds();
  
  if (feeds.length === 0) {
    console.log('No feeds found for The Trusted');
    return;
  }
  
  console.log(`\nFound ${feeds.length} feed(s) from The Trusted:\n`);
  ...
  feeds.forEach(feed => {
    const feedId = feed.title.toLowerCase()
      .replace(/the trusted\s*-?\s*/i, 'the-trusted-')
      .replace(/[^a-z0-9-]/g, '-')
      .replace(/-+/g, '-')
      .replace(/^-|-$/g, '');
