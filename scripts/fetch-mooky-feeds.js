...
// Search for Mooky feeds
async function fetchMookyFeeds() {
  try {
    const headers = generateAuthHeaders();
    
    // Search for music feeds by Mooky
    const searchUrl = `${API_URL}/search/music/byterm?q=mooky&max=20`;
    
    console.log('Fetching Mooky feeds from Podcast Index...');
    ...
    if (!data.feeds || data.feeds.length === 0) {
      ...
      const regularSearchUrl = `${API_URL}/search/byterm?q=mooky&val=music&max=20`;
...
// Main function
async function main() {
  const feeds = await fetchMookyFeeds();
  
  if (feeds.length === 0) {
    console.log('No feeds found for Mooky');
    return;
  }
  
  console.log(`\nFound ${feeds.length} feed(s) from Mooky:\n`);
